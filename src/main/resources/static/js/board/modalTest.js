import { questionerModal } from './qnaQuestionerModal.js';
import { answererModal } from './qnaAnswererModal.js';
...
function requestButton(data) {
    ...
    if (data.status == "대기중") {
        ...
        if (principalUser == null) {
            ...
            // 질문자가 로그인 되어있으면 답변자 유저 정보를 띄움
        } else if (principalUser.id == userId) {
            requestButtonArea.innerHTML = `
            ...
            `;
            if (requestUser != null) {
                selectAnswer(userId, data.id, requestUser);
            }
